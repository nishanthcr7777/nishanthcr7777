...
Matches achievements card size, spacing, and color highlights.
"""
import os, sys, html
...
# bullets: list of (text, color, bold) segments
JOBS = [
    ("TVS Automobile Solutions", GREEN, "SDE Intern", [
        [("AI-assisted", GREEN, True), (" ticketing backend  ", MAIN, False), ("1000+ queries", GOLD, True)],
        [("LLM", PURPLE, True), (" summarization + ", MAIN, False), ("sentiment analysis", CYAN, True)],
        [("REST APIs", CYAN, True), ("  ·  ", DIM, False), ("JWT", GOLD, True), ("  ·  ", DIM, False), ("MySQL", GREEN, True)],
        [("Socket.IO", PURPLE, True), (" real-time  ·  ", MAIN, False), ("RBAC", GOLD, True), (" pipelines", MAIN, False)],
    ]),
    ("BrainGlobe (NIU Group)", CYAN, "Open Source Contributor", [
        [("Python", GOLD, True), (" enhancements for research tooling", MAIN, False)],
        [("Structured PRs", GREEN, True), (" in scientific codebases", MAIN, False)],
    ]),
]
...
def wipe(idx, y, content):
    delay = idx * STAGGER
    svg.append(f'<clipPath id="e{idx}"><rect x="{PAD}" y="{y-2}" height="{LINE_H+2}" width="0">'
               f'<animate attributeName="width" from="0" to="{W-PAD*2}" '
               f'begin="{delay:.3f}s" dur="{ROW_DUR:.2f}s" fill="freeze"/>'
               f'</rect></clipPath>')
    svg.append(f'<g clip-path="url(#e{idx})">{content}</g>')
...
def segs_to_tspans(segments):
    out = []
    for text, color, bold in segments:
        weight = ' font-weight="bold"' if bold else ""
        out.append(f'<tspan fill="{color}"{weight}>{html.escape(text)}</tspan>')
    return "".join(out)
...
for company, color, role, bullets in JOBS:
    header = (f'<text xml:space="preserve" x="{PAD}" y="{y + LINE_H * 0.75:.0f}" font-size="{FONT}">'
              ...
              f'<tspan fill="{DIM}">  \u2014  </tspan>'
              f'<tspan fill="{GOLD}" font-weight="bold">{html.escape(role)}</tspan></text>')
    ...
    for bullet in bullets:
        detail = (f'<text xml:space="preserve" x="{PAD + 14}" y="{y + LINE_H * 0.75:.0f}" '
                  f'font-size="{FONT - 1}">'
                  f'<tspan fill="{DIM}">\u2502 </tspan>{segs_to_tspans(bullet)}</text>')
